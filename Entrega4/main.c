...
int main (int argc, char** argv) {
  ...
  if(argc != 11) {
    ...
    fprintf(stderr, "Uso: heatSim N tEsq tSup tDir tInf iter trab maxD fichS periodoS\n\n");
    return 1;
  }
 ...
 //Impressao dos argumentos introduzidos 
  fprintf(stderr, "\nArgumentos:\n"
  " N=%d tEsq=%.1f tSup=%.1f tDir=%.1f tInf=%.1f iter=%d trab=%d maxD=%lf, fichS=%s, periodoS=%d\n",
  N, tEsq, tSup, tDir, tInf, iter, trab,maxD,fichS,periodoS);

  //Validacao dos argumentos introduzidos
  if(N < 1 || tEsq < 0 || tSup < 0 || tDir < 0 || tInf < 0 || iter < 1 || (N % trab) != 0 || trab < 1 || maxD < 0 || periodoS < 0){
    fprintf(stderr, "\nErro: Argumentos invalidos.\n"
  " Lembrar que N ,trabalhadoras e iteracoes>= 1, trabalhadoras e multiplo de N e temperaturas,maxD e periodoS >= 0 \n\n");
   exit(1);    
  }
  ...
  if((f = fopen(fichS,"r")) == NULL) {  // se nao existir um ficheiro associado a matriz tera de ser inicializada
  ...
  else {                                      // se existir um ficheiro ja criado a matriz sera igual a matriz guardade nesse
    matrix = readMatrix2dFromFile(f,N+2,N+2); //ficheiro
    
    if (matrix == NULL){
      fprintf(stderr, "\nErro: Nao foi possivel alocar memoria para matrix ou matrix.\n\n");
      return -1;
    }

    if(fclose(f) != 0){
      fprintf(stderr, "\nErro: Fechar o ficheiro na main.\n\n");
      return -1;
    }
  }
  ...
  sigset_t set;            // set signals
  if(sigemptyset(&set) == -1) // inicializacao do set de signals
    perror("Erro em sigemptyset\n");
  
  if(periodoS != 0) {
    if(sigaddset(&set,SIGALRM) == -1) //associacao dos signals a estrutura 
      perror("Erro em sigaddset de SIGALRM\n");
  }
  
  if(sigaddset(&set,SIGINT) == -1)
    perror("Erro em sigaddset de SIGINT\n");

  if(pthread_sigmask(SIG_BLOCK,&set,NULL) != 0) { //selecionar a thread que executa signals
    fprintf(stderr, "\nErro: Nao foi possivel mascarar a thread.\n\n");
    return -1;
  }
  ...
  if(sigemptyset(&sa.sa_mask) == -1)
    perror("Erro em sigemptyset\n");
  sa.sa_handler = signalHandler;   //definicao da rotina de tratamento dos signals na estrutura
  sa.sa_flags = 0;

  if(sigaction(SIGINT, &sa,NULL) < 0) //envio dos signals
    perror("sigaction SIGINT na main\n");
  ...
  wait(NULL); // espera que todos os ficheiros filhos terminem para puder eliminar o ficheiro
  
  if(access(fichS,F_OK) != -1) {
    if(unlink(fichS) != 0)  // eliminacao do ficheiro
      perror("Erro ao eliminar fichS");
  }
